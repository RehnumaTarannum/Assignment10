...
<body>
	<%
		if (request.getParameter("email").isEmpty() && request.getParameter("pass").isEmpty()) {
			...
			jt.setAddress(request.getParameter("address"));

			try {
				Class.forName("com.mysql.jdbc.Driver");
				Connection con = DriverManager.getConnection(
						"jdbc:mysql://localhost:3306/sample?autoReconnect=true&useSSL=false", "root",
						"root");
				PreparedStatement st = con.prepareStatement(
						"Insert into members(uname, pass, email, address, number) values ('" + jt.getUsername() + "','"
								+ jt.getPassword() + "','" + jt.getEmail() + "','" + jt.getAddress() + "','" + jt.getContactNo() + "')");
